...
class testForwardRepeat(Lily):
    def setUp(self):
        self.item = BarlinesAndMarkers.Barline(style="heavy-light", repeat="forward")
        self.lilystring = " \\repeat volta 2 {"

class testBackwardRepeat(Lily):
    def setUp(self):
        self.item = BarlinesAndMarkers.Barline(style="light-heavy", repeat="backward")
        self.lilystring = "}"
...
class testBarlineWithEndingStart(Lily):
    def setUp(self):
        self.item = BarlinesAndMarkers.Barline(style="heavy-light",ending=BarlinesAndMarkers.EndingMark())
        self.lilystring = "\\alternative {{"
...
class testMeasureRightBarline(MeasureTests):
    def setUp(self):
        ...
        self.item.addNote(note)
        self.item.AddBarline(BarlinesAndMarkers.Barline(repeat="forward"), location="left")
        self.item.AddBarline(BarlinesAndMarkers.Barline(repeat="backward"), location="right")
        self.lilystring = " \\repeat volta 2 {c' }"
...
class testStaffMeasureFlagValues(unittest.TestCase):
    def setUp(self):
        self.item = StaffNode()
        self.item.AddBarline(item=BarlinesAndMarkers.Barline(repeat="forward"), location="left", measure_id=1)
    ...
    def testBackwardFlag(self):
        self.item.AddBarline(item=BarlinesAndMarkers.Barline(repeat="backward"), location="right", measure_id=1)
        self.assertTrue(hasattr(self.item, "forward_repeats"))
